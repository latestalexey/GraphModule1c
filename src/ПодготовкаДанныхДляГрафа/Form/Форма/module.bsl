...
Процедура КнопкаВыполнитьНажатие(Кнопка)
	...
	ТекстDot = ПодготовитьТекстГрафа(МассивПроцедурФункций, Разбор);
КонецПроцедуры
...
Функция ПодготовитьТекстГрафа(СписокПроцедурФункций, РезультатРВ)
	Текст = "digraph Module1c {";
	Для Каждого ПроцФунк Из СписокПроцедурФункций Цикл
		//1. Сначала запишем параметры
		Текст = Текст + Символы.ПС + ПараметрыЭлемента(ПроцФунк) + ";";
	КонецЦикла;
КонецФункции

Функция ПараметрыЭлемента(ПроцедураФункция)
	Текст = ПроцедураФункция.Имя;
КонецФункции
